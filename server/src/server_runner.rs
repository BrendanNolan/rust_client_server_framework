...
pub async fn run_server<Req: Communicable, Resp: Communicable, Address: ToSocketAddrs, P>(
    address: Address,
    p: P,
    ...
    P: RequestProcessor<Req, Resp> + TriviallyThreadable,
{
    ...
    let (job_dispatcher, jobs_task) = jobs::spawn_jobs_task(p, jobs_buffer_size);
